...
export default function TechnicalImplementationPage() {
  return (
    <div className="container mx-auto pt-36 pb-24 px-4 max-w-4xl">
      ...
      {/* Hero */}
      <div className="mb-14">
        <div className="inline-flex items-center gap-2 rounded-full border border-cyan-500/20 bg-cyan-500/5 px-4 py-1.5 text-sm font-medium text-cyan-400 mb-5">
        ...
        <p className="text-lg text-foreground/70 max-w-2xl leading-relaxed">
          FIRE and ICE are implemented using the Charms protocol — a zkVM-based
          programmable asset system built on Bitcoin and Cardano. This page
          explains what Charms is, how it works, and exactly how Alchemy uses it
          to enforce reserve mechanics on-chain.
        </p>
      </div>
      ...
      <div className="mb-14">
        ...
        <div className="rounded-2xl border border-white/10 bg-secondary/50 p-6 md:p-8 mb-5">
          ...
          <p className="text-foreground/70 leading-relaxed mb-4">
            A single charm is a token, NFT, or instance of arbitrary app state.
            Structurally, it is an entry in a mapping of{" "}
            <code className="rounded bg-white/10 px-1.5 py-0.5 text-xs font-mono text-foreground/90">
              app → data
            </code>{" "}
            living on top of a Bitcoin UTXO. Multiple charms can coexist on the
            same UTXO, creating composable strings of programmable state.
          </p>
          <p className="text-foreground/70 leading-relaxed">
            ...
            mainstream languages (Rust), and it deploys those assets natively on
            other chains — including Cardano — under their standard token
        ...
        <div className="flex flex-wrap gap-3">
          <Link
            ...
            className="inline-flex items-center gap-1.5 rounded-full border border-white/15 bg-white/5 px-4 py-1.5 text-sm text-foreground/70 hover:text-foreground hover:border-white/25 transition-all"
          >
        ...
        <div className="space-y-4">
          {pillars.map((p) => {
            ...
            return (
              <div
                key={p.number}
                className={`rounded-xl border p-5 md:p-6 ${p.color.split(" ")[0]} ${p.color.split(" ")[1]}`}
              >
                <div className="flex items-start gap-4">
                  <div
                    className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-white/10 bg-white/5 ${p.color.split(" ")[2]}`}
                  >
      ...
      <div className="mb-14">
        ...
        <div className="space-y-4">
          <p className="text-foreground/70 leading-relaxed text-sm">
            ...
            <strong className="text-foreground">spell</strong> — Charms-related
            ...
            tuple. The proof is a Groth16 ZK proof attesting to the correctness
            of the spell.
          </p>
          <p className="text-foreground/70 leading-relaxed text-sm">
          ...
          <div className="rounded-xl border border-white/10 bg-secondary/60 p-4">
            <p className="text-xs font-semibold uppercase tracking-widest text-foreground/40 mb-3">
              Spell structure (simplified)
            </p>
            <pre className="text-xs font-mono text-foreground/75 leading-relaxed overflow-x-auto">
              {`version: 11
tx:
  ins:
  ...
  t/<identity>/<vk>:               # ICE app contract`}
      ...
      <div className="mb-14">
        ...
        <p className="text-foreground/70 leading-relaxed text-sm mb-5">
          ...
          transferring. This function is compiled to a zkVM, and a Groth16 proof
          is generated off-chain. Anyone can verify the proof; no one can fake a
          compliant spell without satisfying the contract.
        </p>
        <div className="rounded-xl border border-white/10 bg-secondary/60 p-4 mb-4">
          <p className="text-xs font-semibold uppercase tracking-widest text-foreground/40 mb-3">
    ...
    todo!()
        ...
        <p className="text-sm text-foreground/60 leading-relaxed">
          ...
          spell is accepted and the resulting charms are minted or transferred.
        </p>
      </div>
      ...
      <div className="mb-14">
        ...
        <p className="text-foreground/60 text-sm mb-6">
          ...
          repository implements Charms as Cardano Native Token smart-contracts,
          written in Aiken. Sundial's Sam Delaney is a direct contributor.
        </p>
        <div className="rounded-xl border border-white/10 overflow-hidden mb-5">
          <div className="bg-secondary/70 px-5 py-4 border-b border-white/10">
          ...
          <div className="divide-y divide-white/5">
            {[
              ...
              <div key={row.name} className="flex gap-4 px-5 py-4">
      ...
      <div className="mb-14">
        ...
        <p className="text-foreground/60 text-sm mb-6">
          Charms is the execution layer that makes Alchemy's reserve constraints
        ...
        <div className="space-y-4">
          {[
            ...
            <div
              key={item.step}
              className={`rounded-xl border ${item.color} p-5 flex gap-4`}
            >
              <div
                className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-white/10 bg-white/5 font-bold text-sm ${item.numColor}`}
              >
                {item.step}
              </div>
              <div>
      ...
      <div className="mb-14">
        ...
        <div className="rounded-xl border border-white/10 overflow-hidden">
          <div className="grid grid-cols-[auto_1fr] sm:grid-cols-[160px_1fr] divide-y divide-white/5">
            {stack.map((row) => {
              const Icon = row.icon;
              return (
                <div key={row.layer} className="contents">
                  <div className="flex items-start gap-3 px-5 py-5 bg-secondary/60 border-r border-white/5">
                  ...
                  <div className="px-5 py-5 bg-secondary/30">
        ...
        <div className="grid sm:grid-cols-2 gap-3">
          {[
            ...
            <Link
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center justify-between rounded-xl border border-white/10 bg-secondary/50 px-4 py-3.5 hover:border-white/20 transition-colors group"
            >
              <div>
              ...
              <ExternalLink className="h-4 w-4 text-foreground/30 group-hover:text-foreground/60 transition-colors shrink-0" />
            </Link>
          ))}
        </div>
      </div>
